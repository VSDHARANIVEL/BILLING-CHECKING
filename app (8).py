"""
=============================================================
  BillPro — Flask Backend  (Supabase Edition, Fixed)
=============================================================
  ENV VARIABLES to set in Railway / Render:

  DATABASE_URL   = (see below — use the DIRECT connection URL)
  SECRET_KEY     = any-long-random-string
  MGR_USERNAME   = manager
  MGR_PASSWORD   = manager123
  ...
  CORRECT format:
    postgresql://postgres:mypassword@db.abcdef.supabase.co:5432/postgres?sslmode=require

  ❌ WRONG (do NOT use pooler URL with port 6543):
    postgresql://postgres.xxxx:password@aws-1-ap-south-1.pooler.supabase.com:6543/postgres
=============================================================
"""
...
def _build_dsn(url):
    ...
    - If pooler URL (port 6543) is used, switch to direct port 5432
    ...
    if _pool is None:
        if not DATABASE_URL:
            raise RuntimeError(
                ...
                "Add it in your Railway/Render environment variables. "
...
def get_db():
    ...
    conn = pool.getconn()
    try:
        # Reconnect if connection went stale
        if conn.closed:
...
def init_db():
    for attempt in range(3):
        try:
            ...
            time.sleep(2)
    print("❌ Could not initialise DB — check DATABASE_URL")
...
def create_bill():
    ...
    if not items:  return jsonify({'error': 'No items in bill'}), 400
    ...
    with get_db() as conn:
        for it in items:
            ...
            sub    = safe_float(prod['price']) * qty
            total += sub
            enriched.append({'code':code,'name':prod['name'],
                             'price':safe_float(prod['price']),'qty':qty,'sub':sub})

        bill = q1(conn,
            """INSERT INTO bills(customer_name,customer_phone,customer_email,
        ...
        for it in enriched:
            qrun(conn,
                """INSERT INTO bill_items(bill_id,product_code,product_name,
                   quantity,unit_price,subtotal) VALUES(%s,%s,%s,%s,%s,%s)""",
                (bill_id,it['code'],it['name'],it['qty'],it['price'],it['sub']))
            qrun(conn, 'UPDATE products SET stock=stock-%s WHERE code=%s',
                 (it['qty'], it['code']))
...
def daily_sales():
    ...
    with get_db() as conn:
        bills = q(conn,
            """SELECT b.id, b.customer_name, b.customer_phone,
                      b.worker_number, b.worker_name, b.total_amount,
                      b.bill_date,
                      COUNT(bi.id) AS item_count,
                      SUM(bi.quantity) AS total_pieces
               FROM bills b
               LEFT JOIN bill_items bi ON bi.bill_id = b.id
               WHERE DATE(b.bill_date AT TIME ZONE 'Asia/Kolkata') = %s
               GROUP BY b.id ORDER BY b.id DESC""", (rdate,))
        summary = q1(conn,
            """SELECT COUNT(*) AS total_bills,
                      COALESCE(SUM(total_amount),0) AS total_sales,
                      COUNT(DISTINCT customer_phone) AS unique_customers
               FROM bills
               WHERE DATE(bill_date AT TIME ZONE 'Asia/Kolkata') = %s""", (rdate,))
        top_products = q(conn,
            """SELECT bi.product_name,
               ...
               WHERE DATE(b.bill_date AT TIME ZONE 'Asia/Kolkata') = %s
               GROUP BY bi.product_name ORDER BY units DESC LIMIT 5""", (rdate,))
    for r in bills:
        if r.get('bill_date'): r['bill_date'] = str(r['bill_date'])
    return jsonify({
        'date': rdate,
        ...
        'summary': {
            ...
            'total_sales':       safe_float(summary['total_sales']),
            'unique_customers':  int(summary['unique_customers'] or 0),
        },
        'top_products': top_products
    })

# ── Monthly Sales Report (supervisor + manager) ───────────────
...
def monthly_sales():
    ...
    with get_db() as conn:
        ...
        summary = q1(conn,
            """SELECT COUNT(*) AS total_bills,
               ...
               WHERE EXTRACT(YEAR FROM bill_date)=%s
                 AND EXTRACT(MONTH FROM bill_date)=%s""", (year, month))
        # Daily breakdown
        daily = q(conn,
            """SELECT DATE(bill_date AT TIME ZONE 'Asia/Kolkata') AS sale_date,
                      COUNT(*) AS bills,
                      SUM(total_amount) AS sales
               FROM bills
               WHERE EXTRACT(YEAR FROM bill_date)=%s
                 AND EXTRACT(MONTH FROM bill_date)=%s
               GROUP BY sale_date ORDER BY sale_date""", (year, month))
        # Worker-wise sales
        workers = q(conn,
            """SELECT worker_number, worker_name,
               ...
               WHERE EXTRACT(YEAR FROM bill_date)=%s
                 AND EXTRACT(MONTH FROM bill_date)=%s
                 AND worker_number IS NOT NULL AND worker_number != ''
               GROUP BY worker_number, worker_name ORDER BY sales DESC""", (year, month))
        # Top products
        top_products = q(conn,
            """SELECT bi.product_name,
                      SUM(bi.quantity) AS units,
               ...
               FROM bill_items bi
               JOIN bills b ON b.id = bi.bill_id
               WHERE EXTRACT(YEAR FROM b.bill_date)=%s
                 AND EXTRACT(MONTH FROM b.bill_date)=%s
               GROUP BY bi.product_name ORDER BY units DESC LIMIT 10""", (year, month))
        # Monthly attendance (manager gets all; supervisor gets their workers)
        att_rows = []
        if session.get('is_manager'):
            workers_list = q(conn, 'SELECT number, name FROM workers ORDER BY number')
        else:
            sid = session['supervisor_id']
            workers_list = q(conn,
                """SELECT w.number, w.name FROM workers w
        ...
        for w in workers_list:
            ...
            att_rows.append({'number':w['number'],'name':w['name'],
                             'present':P,'absent':A,'half':H,'leave':L,
                             'total_present': P + H*0.5})
    for r in daily:
        if r.get('sale_date'): r['sale_date'] = str(r['sale_date'])
    return jsonify({
        ...
        'worker_sales':    workers,
        'top_products':    top_products,
        'attendance':      att_rows,
    })

# ── Original reports endpoint (summary cards) ─────────────────
...
def reports():
    with get_db() as conn:
        ...
        top_prods    = q(conn,
            """SELECT product_name, SUM(quantity) AS units, SUM(subtotal) AS revenue
               FROM bill_items GROUP BY product_name
    ...
    return jsonify({
        ...
        'total_incentives': safe_float(total_inc),
...
def customer_lookup():
    ...
    with get_db() as conn:
        bill = q1(conn,
            'SELECT * FROM bills WHERE customer_phone=%s ORDER BY id DESC LIMIT 1',(phone,))
        if not bill: return jsonify({'error': 'No bills found for this phone'}), 404
        items = q(conn, 'SELECT * FROM bill_items WHERE bill_id=%s', (bill['id'],))
        count = qval(conn, 'SELECT COUNT(*) FROM bills WHERE customer_phone=%s', (phone,))
...
def adjust_incentive():
    ...
    with get_db() as conn:
        qrun(conn,
            ...
            (adj, adj, wnum))
        qrun(conn,
            'INSERT INTO incentive_adjustments(worker_number,adjustment,note,created_at) VALUES(%s,%s,%s,NOW())',
            (wnum, adj, note))
    return jsonify({'message': f'Adjusted {adj} for worker {wnum}'})
...
def sup_register():
    ...
    try:
        with get_db() as conn:
            qrun(conn, 'INSERT INTO supervisors(username,password) VALUES(%s,%s)', (u,hash_pw(p)))
    except psycopg2.errors.UniqueViolation:
...
def sup_login():
    ...
    with get_db() as conn:
        row = q1(conn,
            'SELECT * FROM supervisors WHERE username=%s AND password=%s', (u,hash_pw(p)))
...
def sup_status():
    if session.get('is_manager'):
        return jsonify({'logged_in':True,'is_supervisor':True,'is_manager':True,'username':MGR_USER})
    if session.get('supervisor_id'):
        return jsonify({'logged_in':True,'is_supervisor':True,'is_manager':False,
                        'username':session.get('supervisor_name','')})
    return jsonify({'logged_in':False,'is_supervisor':False,'is_manager':False,'username':''})
...
def mgr_login():
    d = request.json or {}
    if str(d.get('username','')).strip() != MGR_USER or str(d.get('password','')).strip() != MGR_PASS:
...
def mgr_supervisors():
    if not session.get('is_manager'): return jsonify({'error': 'Manager access required'}), 403
    with get_db() as conn:
        sups = q(conn, 'SELECT * FROM supervisors ORDER BY username')
        result = []
        for s in sups:
            workers = q(conn,
                'SELECT w.* FROM workers w JOIN supervisor_workers sw ON sw.worker_number=w.number WHERE sw.supervisor_id=%s ORDER BY w.number',
                (s['id'],))
...
def unassigned_workers():
    ...
    with get_db() as conn:
        rows = q(conn,
            'SELECT * FROM workers WHERE number NOT IN (SELECT worker_number FROM supervisor_workers) ORDER BY number')
    return jsonify(rows)
...
def assign_worker():
    ...
    try:
        with get_db() as conn:
            qrun(conn, 'INSERT INTO supervisor_workers(supervisor_id,worker_number) VALUES(%s,%s)',
                 (int(d.get('supervisor_id',0)), str(d.get('worker_number','')).strip()))
    except psycopg2.errors.UniqueViolation:
...
def unassign_worker():
    ...
    with get_db() as conn:
        qrun(conn, 'DELETE FROM supervisor_workers WHERE worker_number=%s', (wn,))
        ...
        if session.get('is_manager'):
            rows = q(conn,
                """SELECT w.number, w.name,
                     ...
                     a.status
                   FROM workers w
                   LEFT JOIN attendance a ON a.worker_number=w.number AND a.att_date=%s
                   ORDER BY w.number""", (att_date,))
        else:
            sid = session['supervisor_id']
            rows = q(conn,
                """SELECT w.number, w.name, NULL AS supervisor, a.status
                   ...
                   JOIN supervisor_workers sw ON sw.worker_number=w.number
                   LEFT JOIN attendance a ON a.worker_number=w.number AND a.att_date=%s
                   WHERE sw.supervisor_id=%s ORDER BY w.number""", (att_date, sid))
...
def mark_attendance():
    ...
    if status not in ('P','A','H','L'): return jsonify({'error': 'Invalid status'}), 400
    with get_db() as conn:
        qrun(conn,
            """INSERT INTO attendance(worker_number,att_date,status) VALUES(%s,%s,%s)
               ON CONFLICT(worker_number,att_date) DO UPDATE SET status=EXCLUDED.status""",
            (wnum, att_dt, status))
    return jsonify({'message': f'{wnum} → {status} on {att_dt}'})
    ...
    with get_db() as conn:
        rows = q(conn,
            """SELECT w.number, w.name, COALESCE(a.status,'U') AS status
               FROM workers w LEFT JOIN attendance a
...
def monthly_attendance():
    ...
    with get_db() as conn:
        ...
        for w in workers:
            recs = q(conn,
                """SELECT status FROM attendance WHERE worker_number=%s
                   AND EXTRACT(YEAR FROM att_date)=%s AND EXTRACT(MONTH FROM att_date)=%s""",
                (w['number'], year, month))
            P = sum(1 for r in recs if r['status']=='P')
            A = sum(1 for r in recs if r['status']=='A')
            H = sum(1 for r in recs if r['status']=='H')
            ...
            rows.append({'number':w['number'],'name':w['name'],
                         'present':P,'absent':A,'half':H,'leave':L,'total_present':P+H*0.5})
    return jsonify({'rows': rows})
...
def download_attendance():
    if not session.get('supervisor_id') and not session.get('is_manager'):
        return jsonify({'error': 'Login required'}), 401
    if not OPENPYXL_OK: return jsonify({'error': 'openpyxl not installed'}), 500
    year  = int(request.args.get('year',  date.today().year))
    month = int(request.args.get('month', date.today().month))
    with get_db() as conn:
        workers = q(conn, 'SELECT number, name FROM workers ORDER BY number')
    ...
    wb = openpyxl.Workbook(); ws = wb.active; ws.title = f"{year}-{month:02d}"
...
if __name__ == '__main__':
    init_db()
    print("=" * 60)
    print("  BillPro — Supabase Edition (Fixed)")
    print(f"  Open → http://localhost:{PORT}")
    print(f"  Manager: {MGR_USER} / {MGR_PASS}")
    print("=" * 60)
